...
const EmployeeCard = ({ employee, onExecutiveSelect, depth = 0 }) => {
    // Assuming depth increments to control the visual hierarchy
    const [showReports, setShowReports] = useState(false);
    const [selectedReports, setSelectedReports] = useState([]);

    const handleToggleReports = useCallback(() => {
        setShowReports(!showReports);
        if (!showReports) { // If we are about to show the reports, update the selectedReports state
            setSelectedReports(employee.direct_reports || []);
        } else { // If hiding, reset to show no reports
            setSelectedReports([]);
        }
    }, [showReports, employee.direct_reports]);
        ...
        <div className='flex flex-col items-center'>
            <Card className={`bg-white shadow-lg rounded-xl my-4 w-[300px] h-[150px]`}>
                <CardHeader className="p-4 flex  rounded-xl  justify-between items-center bg-gray-100 h-full relative">
                    ...
                    {employee?.direct_reports_count > 0 ? (
                        <Badge className="cursor-pointer text-white bg-black hover:bg-[#121212] absolute bottom-[-10px] gap" onClick={handleToggleReports}>
                            {employee.direct_reports_count}
                            <div className={showReports ? '' : 'rotate-180'}>
            ...
            {selectedReports.length > 0 && (
                <>
                    ...
                    <div className={`flex gap-8 items-center ml-${depth * 20}`}>
                        {selectedReports.map((report, index) => (
                            <EmployeeCard key={index} employee={report} onExecutiveSelect={onExecutiveSelect} depth={depth + 1} />
                        ))}
                    </div>
                </>
            )}
